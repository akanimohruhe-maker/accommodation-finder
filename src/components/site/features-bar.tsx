 ...
 *
 * Each icon has a different hover animation (see globals.css):
 *  1. VerifiedMark    — .icon-anim-tilt   (soft clockwise tilt)
 *  2. SecureMark      — .icon-anim-wiggle (gentle shackle shake)
 *  3. AffordableMark  — .icon-anim-spin   (slow coin spin around Y axis)
 *  4. StudentMark     — .icon-anim-bounce (vertical bob)
 *  5. SupportMark     — .icon-anim-tick   (tick-tock rotation)
 */
...
const features = [
  { Icon: VerifiedMark,   animClass: "icon-anim-tilt",   title: "Verified properties",  desc: "100% of listings pass our 32-point verification check." },
  { Icon: SecureMark,     animClass: "icon-anim-wiggle", title: "Secure booking",        desc: "Encrypted payments and lease signing. No wire fraud risk." },
  { Icon: AffordableMark, animClass: "icon-anim-spin",   title: "Affordable price",      desc: "Direct-from-landlord rates. No hidden agency fees." },
  { Icon: StudentMark,    animClass: "icon-anim-bounce", title: "Student-friendly",     desc: "Locations vetted for proximity to UK university campuses." },
  { Icon: SupportMark,   animClass: "icon-anim-tick",   title: "24/7 support",          desc: "Our team is on hand throughout your tenancy. Always." },
];

export function FeaturesBar() {
  return (
    <section id="about" className="bg-bg-elevated py-20 lg:py-24 border-y border-line relative overflow-hidden">
      {/* Yellow bar decoration (uploaded brand asset) */}
      <div
        className="absolute inset-x-0 top-0 h-1.5 bg-cover bg-center opacity-90"
        style={{ backgroundImage: "url('/images/yellow-bar.png')" }}
        aria-hidden="true"
      />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* Features row — each icon container is a .group so the .icon-anim-X
            rule fires on hover. Each icon has a different animation. */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-6 gap-y-10">
          {features.map(({ Icon, animClass, title, desc }) => (
            <div key={title} className="group">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-brand/8 border border-brand/15 transition-colors group-hover:bg-brand/12">
                {/* The SVG is wrapped in a span with the animation class —
                    this is what rotates/wiggles/bounces on hover. */}
